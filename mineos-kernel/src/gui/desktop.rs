...
use crate::gui::window::{WindowManager, AppType};
use crate::gui::taskbar::Taskbar;
use alloc::format;
    ...
    loop {
        ...
        if let Some(event) = keyboard::read_key() {
            if let Some(ch) = event.character {
                handle_keyboard_input(&mut wm, ch, &event);
            }
        }
...
/// Draw the desktop wallpaper (gradient)
    ...
    match app_type {
        ...
        AppType::FileManager => {
            ...
            for (i, name) in files.iter().enumerate() {
                ...
                if is_folder {
                    fb.fill_rounded_rect(fx + 16, fy + 4, 40, 28, 3, palette::ACCENT);
                } else {
                    fb.fill_rounded_rect(fx + 20, fy + 2, 32, 32, 2, Color::new(30, 30, 45));
                    fb.draw_rect(fx + 20, fy + 2, 32, 32, palette::TEXT_SECONDARY);
                }
...
fn handle_keyboard_input(wm: &mut WindowManager, ch: char, event: &keyboard::KeyEvent) {
    if event.ctrl {
        match ch {
            't' | 'T' => { wm.open("Terminal", 100, 50, 600, 400, AppType::Terminal); }
            'e' | 'E' => { wm.open("Text Editor", 150, 60, 650, 450, AppType::TextEditor); }
            'w' | 'W' => {
