...
impl Window {
    ...
    pub fn draw_frame(&self, fb: &mut FrameBuffer) {
        if self.minimized || !self.visible { return; }

        fb.fill_rounded_rect(self.x, self.y, self.width, self.height, 6, palette::WINDOW_BG);
        fb.draw_rect(self.x, self.y, self.width, self.height,
        ...
        fb.draw_text(self.x + 12, self.y + (TITLEBAR_HEIGHT - 16) / 2, &self.title, palette::TEXT_PRIMARY, 1);
    ...
    pub fn content_rect(&self) -> (usize, usize, usize, usize) {
        (self.x + 1, self.y + TITLEBAR_HEIGHT + 1, self.width - 2, self.height - TITLEBAR_HEIGHT - 2)
    }

    pub fn is_in_titlebar(&self, px: usize, py: usize) -> bool {
        px >= self.x && px < self.x + self.width && py >= self.y && py < self.y + TITLEBAR_HEIGHT
    }

    pub fn is_on_close_btn(&self, px: usize, py: usize) -> bool {
        let btn_y = self.y + (TITLEBAR_HEIGHT - BTN_SIZE) / 2;
        let close_x = self.x + self.width - BTN_SIZE - 8;
        px >= close_x && px < close_x + BTN_SIZE && py >= btn_y && py < btn_y + BTN_SIZE
    }
    ...
    pub fn is_on_minimize_btn(&self, px: usize, py: usize) -> bool {
        ...
        let max_x = close_x - BTN_SIZE - 4;
        let min_x = max_x - BTN_SIZE - 4;
        px >= min_x && px < min_x + BTN_SIZE && py >= btn_y && py < btn_y + BTN_SIZE
    }
    ...
    pub fn is_on_maximize_btn(&self, px: usize, py: usize) -> bool {
        ...
        let close_x = self.x + self.width - BTN_SIZE - 8;
        let max_x = close_x - BTN_SIZE - 4;
        px >= max_x && px < max_x + BTN_SIZE && py >= btn_y && py < btn_y + BTN_SIZE
    }
...
impl WindowManager {
    pub fn new() -> Self {
        WindowManager { windows: Vec::new(), next_id: 1, next_z: 1 }
    }
    ...
    pub fn close(&mut self, id: usize) { self.windows.retain(|w| w.id != id); }

    pub fn focus(&mut self, id: usize) {
        for w in &mut self.windows {
            w.focused = w.id == id;
            if w.id == id { w.z_index = self.next_z; self.next_z += 1; }
        }
    }
    ...
    pub fn draw_all(&self, fb: &mut FrameBuffer) {
        ...
        for win in sorted { win.draw_frame(fb); }
    }
